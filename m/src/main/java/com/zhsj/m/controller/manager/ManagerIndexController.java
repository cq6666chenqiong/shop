...
public class ManagerIndexController implements Constants{
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(ManagerIndexController.class);
    @Autowired
    private MerchantAccountService merchantAccountService;
    @RequestMapping("")
    public String index() {
        return "/managerPages/index";
    }
    @RequestMapping("/home")
    public String home() {
            ...
            if(verifyCode!=null &&  imagecode !=null &&verifyCode.toUpperCase().equalsIgnoreCase(imagecode.toUpperCase())){//验证码
                ...
                if(user!=null){
                    if(pwd.equalsIgnoreCase(user.getPassword())){
                        ...
                        CookieUtils.writeCookie(response,desKey,MANAGER_USER_INFO, JsonUtils.toJSON(userInfo));
